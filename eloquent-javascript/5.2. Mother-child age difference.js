...
Using the provided data set of ancestors, compute the average age difference between mothers and children 
(the age of the mother when the child is born). 

Suggestions: You can use the average function defined earlier in this chapter. 

The byName object, which makes it easy to find a person’s object from their name, might be useful here.

Because not all elements in the ancestry array produce useful data (we can’t compute the age difference 
unless we know the birth date of the mother), we will have to apply filter in some manner before calling average. 
You could do it as a first pass, by defining a hasKnownMother function and filtering on that first. Alternatively, 
you could start by calling map and in your mapping function return either the age difference or null if no 
mother is known. Then, you can call filter to remove the null elements before passing the array to average. */
...
// My answer before incorporating Eloquent JavaScript suggestions:
var sumOfDifferences = 0;
var sets = 0;

for (var child = 0; child < ancestry.length; child++) {
  for (var person = 0; person < ancestry.length; person++) { 
    if (ancestry[child].mother == ancestry[person].name) { 
        sumOfDifferences += ancestry[child].born - ancestry[person].born; 
        sets += 1;
    }
  }         
}

debug(sumOfDifferences/sets); // prints 31.2 
