...
an element in the array and returns an object that maps group names to arrays of 
group members. 

Hints: During the grouping process, keep an object that associates century names (numbers) 
with arrays of either person objects or ages. Since we do not know in advance what categories 
we will find, we’ll have to create them on the fly. For each person, after computing their 
century, we test whether that century was already known. If not, add an array for it. Then 
...
function computesCentury(ancestor) {
  return Math.ceil(ancestor.died/100); 
}

function groupBy(ancestry, computesCentury) {
    var grouped = {};
  
    ancestry.forEach(function(ancestor) {
        var century = computesCentury(ancestor); 
        var age = ancestor.died - ancestor.born; 
        
        if (century in grouped) { 
            grouped[century].push(age); 
        } else { 
            grouped[century] = [age]; 
        }
    });
...
var grouped = groupBy(ancestry, computesCentury);

for (var group in grouped) {
    debug(group + ": " + average(grouped[group]));
}
